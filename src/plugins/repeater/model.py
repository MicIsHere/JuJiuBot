...
###
    ...
    @ staticmethod
    def speak() -> Optional[Tuple[int, int, List[Message], bool, int]]:
        '''
        主动发言，返回当前最希望发言的 bot 账号、群号、发言消息 List、是否为戳一戳、要戳一戳的账号，也有可能不发言
        '''

        # 太难写了qaq
        # 直接做一个简单的Lagrange适配好了
        is_poke = False
        poke_id = 0
        ...
        for group_id, group_msgs in popularity:
            ...
            return (bot_id, group_id, speak_list, is_poke, poke_id)
    ...
    def _context_find(self) -> Optional[Tuple[List[str], str]]:
        ...
        for answer in context['answers']:
            ...
            if sample_msg.startswith("[CQ:xml"):
                continue
            if sample_msg.startswith("[CQ:at,qq=0"): # Lagrange适配，防止牛牛学
                continue
            if sample_msg == "[CQ:image,file=https://multime]": # 异常数据，防止牛牛学
                continue
            if sample_msg.startswith("该消息只支持") or sample_msg.startswith("[暂不支持该消息类型") or sample_msg.startswith("该消息类型暂不"): # Lagrange适配，防止牛牛学
                continue
            if '\n' in sample_msg:
                continue
