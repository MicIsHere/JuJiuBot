...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    for item in answers:
        ...
        if str(msg)[:30] == "[CQ:image,file=https://multime]":
            logger.info("find bad message [{}], blocked.".fotmat(str(msg)[:30]))
            return
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if Chat.ban(event.group_id, event.self_id, raw_message, str(event.user_id)):
        await ban_msg.finish([random.choice(SORRY_WORDS)])
...
async def _(bot: Bot, event: GroupRecallNoticeEvent, state: T_State):
    ...
    if Chat.ban(event.group_id, event.self_id, raw_message, str(f'recall by {event.operator_id}')):
        await ban_recalled_msg.finish([random.choice(SORRY_WORDS)])


async def message_is_ban(bot: Bot, event: GroupMessageEvent, state: T_State) -> bool:
    return event.get_plaintext().strip() == '不可以说这个'
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if Chat.ban(event.group_id, event.self_id, '', str(event.user_id)):
        await ban_msg_latest.finish([random.choice(SORRY_WORDS)])
...
async def speak_up():
    ...
    bot_id, group_id, messages, is_poke, poke_id = ret

    for msg in messages:
    
        # Lagrange适配，如果为戳一戳就进行发送
        if is_poke:
            logger.info("ready send poke at user [{}] in group [{}] ".format(poke_id, group_id))
        ...
        if msg.extract_plain_text() == "[CQ:image,file=https://multime]":
            logger.info('check to bad message[MULTIME], blocked.')
            return

        # 进入正常消息处理
        logger.info(
            'bot [{}] ready to speak_up [{}] to group [{}], and type is [{}]'.format(
                bot_id, msg, group_id, type(msg)))
